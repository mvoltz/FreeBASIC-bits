...
//!      hinted, this is very import for quality. (This is not possible with baked fonts.)
//
...
//!      if you don't do this, stb_truetype is forced to do the conversion on every call.
//
...
//!   given file is in a general way. I provide an API for this, but I don't recommend it.
//
//
...
//! in here: a 1-channel bitmap that is width x height. stride_in_bytes is
//! the distance from one row to the next (or 0 to mean they are packed tightly together).
//! "padding" is the amount of padding to leave between each character
//! (normally you want '1' for bitmaps you'll use as textures with bilinear filtering).
...
//! (use font_index=0 if you don't know what that is).
//! It creates num_chars_in_range bitmaps for characters with
//! unicode values starting at first_unicode_char_in_range and increasing.
//! Data for how to render them is stored in chardata_for_range;
...
//! This will usually create a better-packed bitmap than multiple calls to stbtt_PackFontRange.
//! Note that you can call this multiple times within a single PackBegin/PackEnd.
...
//! This function sets the amount of oversampling for all following calls to stbtt_PackFontRange(s)
//! or stbtt_PackFontRangesGatherRects for a given! pack context.
//! The default (no oversampling) is achieved by h_oversample=1 and v_oversample=1.
//! The total number of pixels required is h_oversample*v_oversample larger than the default; for example, 2x2
...
//! If you more control over the packing of multiple fonts, or if you want to pack custom data into a font texture, 
//! take a look at the source to of stbtt_PackFontRanges() and create a custom version 
//! using these functions, e.g. call GatherRects multiple times,
//! building up a single array of rects, then call PackRects once,
//! then call RenderIntoRects repeatedly. This may result in a
//! better packing than calling PackFontRanges multiple times (or it may not).
...
//! Each font has a sequential index number starting from 0.
//! Call this function to get the font offset for a given index
//! it returns -1 if the index is out of range.
//! A regular .ttf! file will only define one font and it always be at offset 0, 
//! so it will! return '0' for index 0, and -1 for all other indices.
...
//! You must allocate the stbtt_fontinfo yourself, and stbtt_InitFont will fill it out.
//! You don't need to do anything special to free it, because the contents are pure
//! value data with no additional data structures. Returns 0 on failure.
STBTT_DEF int stbtt_InitFont(stbtt_fontinfo *info, const unsigned char *data, int offset);

//! CHARACTER TO GLYPH-INDEX CONVERSIOn

//! If you're going to perform multiple operations on the same character and you want a speed-up,
//! call this function with the character you're going to process, 
//! then use glyph-based functions instead of the codepoint-based functions.
...
//! in other words, it's equivalent to calling stbtt_GetFontVMetrics and computing:
...
STBTT_DEF void stbtt_GetGlyphHMetrics   (const stbtt_fontinfo *info, int glyph , int *advanceWidth, int *leftSideBearing);
STBTT_DEF int  stbtt_GetGlyphKernAdvance(const stbtt_fontinfo *info, int glyph1, int glyph2);
STBTT_DEF int  stbtt_GetGlyphBox        (const stbtt_fontinfo *info, int glyph , int *x0, int *y0, int *x1, int *y1);
...
//! The shape is a series of countours.
//! Each one starts with a STBTT_moveto, then consists of a series of mixed
//! STBTT_lineto and STBTT_curveto segments.
//! A lineto draws a line from previous endpoint to its x,y;
//! a curveto draws a quadratic bezier from previous endpoint to its x,y, using cx,cy as the bezier control point.
...
//! specified character/glyph at the specified scale into it, with antialiasing.
...
//! in the form of 'output', with row spacing of 'out_stride' bytes.
//! the bitmap is clipped to out_w/out_h bytes.
//! Call stbtt_GetCodepointBitmapBox to get the width and height and positioning info for it first.
    ...
    for (i=0; i < num_verts; ++i) {
      switch (vertices[i].type) {
        ...
        case STBTT_vline:
          ...
          break;
        case STBTT_vcurve:
          stbtt__tesselate_curve(points, &num_points,
                                 x, y,
...
STBTT_DEF void stbtt_Rasterize(stbtt__bitmap * result,
   ...
   stbtt__point * windings = stbtt_FlattenCurves(vertices, num_verts, 
                                                 flatness_in_pixels / scale, /* objspace_flatness */
                                                 &winding_lengths, 
                                                 &winding_count,
                                                 userdata);
   if (windings) {
      stbtt__rasterize(result,  
                       windings, winding_lengths, winding_count,
                       scale_x, scale_y,
                       shift_x, shift_y,
                       x_off, y_off,
                       invert,
...
static float stbtt__oversample_shift(int oversample) {
   ...
   //! which shifts phase by (oversample - 1)/2 pixels in oversampled space.
   //! We want to shift in the opposite direction to counter this.
